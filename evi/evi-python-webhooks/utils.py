...
def validate_hmac_signature(payload: str, timestamp: str, signature: str) -> None:
    ...
    the result using the HMAC-SHA256 algorithm with the webhook signing key as the secret.

    This key can be provisioned via the Hume API.
    ...
    Raises:
        ValueError: If the signature is invalid or the webhook signing key is missing.
    """
    signing_key = os.environ.get("HUME_WEBHOOK_SIGNING_KEY")
    if not signing_key:
        print("Error: HUME_WEBHOOK_SIGNING_KEY is not set in environment variables.")
