...
async def observer_message_handler(message: dict) -> None:
    ...
    if msg_type == "chat_metadata":
        print(f"[OBSERVER] Chat ID: {message.get('chat_id')}, Chat Group ID: {message.get('chat_group_id')}")
...
async def control_plane_demo(client: AsyncHumeClient, chat_id: str, api_key: str) -> None:
...
async def main() -> None:
    """Main function that establishes the data plane connection and demonstrates control plane features.
    ...
    async def on_message(message: SubscribeEvent):
        ...
        if message.type == "chat_metadata":
            ...
            print(f"[DATA_PLANE] Chat ID: {message.chat_id}, Chat Group ID: {message.chat_group_id}")
    ...
    async with client.empathic_voice.chat.connect_with_callbacks(
        ...
        print("[DATA_PLANE] You can now speak to the assistant. The control plane will demonstrate:")
