...
describe( "connect to EVI", () =>
{
  ...
  it(
    ...
    async () =>
    {
      ...
      const expectations = [
        { key: "system_prompt", value: sessionSettings.systemPrompt, label: "systemPrompt" },
        { key: "voice_id", value: sessionSettings.voiceId, label: "voiceId" },
        { key: "custom_session_id", value: sessionSettings.customSessionId, label: "customSessionId" },
        { key: "event_limit", value: sessionSettings.eventLimit, label: "eventLimit" },
      ];
...
function waitForSocketOpen ( socket: any ): Promise<void>
{
  return new Promise( ( resolve, reject ) =>
  {
    ...
    socket.on( "close", ( event: any ) =>
      reject( new Error( `Socket closed before opening. Code: ${ event?.code }, Reason: ${ event?.reason }` ) )
    );
  } );
}
...
async function fetchChatEvents ( chatId: string ): Promise<Hume.empathicVoice.ReturnChatEvent[]>
{
